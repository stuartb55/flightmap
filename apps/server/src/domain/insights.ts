...
export function coverageGridCellFromIndices(
  latitudeIndex: number,
  longitudeIndex: number
): CoverageGridCell {
  if (!Number.isInteger(latitudeIndex) || latitudeIndex < 0 || latitudeIndex > 3599) {
    throw new RangeError("latitude index must be an integer between 0 and 3599");
  }
  if (!Number.isInteger(longitudeIndex) || longitudeIndex < 0 || longitudeIndex > 7199) {
    throw new RangeError("longitude index must be an integer between 0 and 7199");
  }
...
export function coverageGridCell(latitude: number, longitude: number): CoverageGridCell {
  ...
  return coverageGridCellFromIndices(latitudeIndex, longitudeIndex);
}
