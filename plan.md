...
explains: NIC / NACp, NACv / SIL, containment radius, and ADS-B version
(`AircraftDetailPanel.tsx:385`–`:388`), the ADS-B versus MLAT source (`:382`),
emitter category (`:409`), the retention model, and "detailed track expired".
The shortcut list exists only inside the `?` dialog, as a module-private const
(`KeyboardShortcuts.tsx:24`).

**Approach.** A `/help` route, added to the known-path list in `App.tsx:46` and
lazily loaded like every other page. Five sections, each with a stable `id` so
the rest of the app can deep-link into it (`/help#glossary`,
`/help#retention`):

1. *Keyboard shortcuts* — rendered from the same array the `?` dialog uses.
2. *Field glossary* — every jargon term the detail panel and the map legend
   show, in the wording the UI shows it.
3. *Retention and expiry* — what `historyRetentionDays` means, which aggregates
   outlive detailed tracks, and why a record can exist with no track behind it
   (the behaviour item 25 shipped).
4. *Units and exports* — the unit preference set, and that CSV/GeoJSON exports
   stay in ft/kt/nm regardless.
5. *Map data provenance* — tile and style providers, the waypoint disclaimer
   from `default-waypoints.ts` ("display-only, not for navigation"), and the
   airport source once item 14 lands.

*One source for the shortcuts.* Export the `shortcuts` const from
`KeyboardShortcuts.tsx` (or lift it to `lib/keyboard-shortcuts.ts` if the import
direction gets awkward) and render both the dialog and the help table from it.

*One source for the glossary.* Put the terms in
`apps/web/src/lib/glossary.ts` as an ordered `term → definition` map, and add a
second exported list of terms considered self-explanatory (Altitude, Speed,
Track, …). The test renders `AircraftDetailPanel` with a fully populated fixture,
collects the text of every `<dt>`, and asserts each one appears in the glossary
or on the self-explanatory list. A new jargon field then fails the test until it
is either documented or explicitly declared plain — which is the point.

*Offline.* The page must render with the network down, because that is exactly
when someone opens it. The mechanics already in place: workbox precaches all
built JS (`vite.config.ts:71`), and navigations use NetworkFirst with
`precacheFallback: { fallbackURL: '/index.html' }`, so an offline hit on `/help`
serves the precached shell and the client router renders the page from
`pathname`. The requirement this creates is that the help page fetches nothing —
no `api.*` call, no runtime-config dependency beyond what the shell already
carries. Assert it rather than assume it.

*Nav placement.* The primary nav already carries six items
(`AppShell.tsx:8`) and the mobile bar renders the same six. A seventh pushes it
over. Help is therefore not a primary nav item: reach it from the `?` dialog
(a "Full help" link), from the command palette via `lib/app-commands.ts`, and
from a small icon beside the existing shortcut button in the shell header. If a
nav entry is wanted later, it belongs in an overflow, not inline.

**Files.** New `apps/web/src/pages/HelpPage.tsx`, new
`apps/web/src/lib/glossary.ts`, new `apps/web/src/styles/help.css`, `App.tsx`,
`AppShell.tsx`, `KeyboardShortcuts.tsx` (export the shortcut list),
`lib/app-commands.ts`, cross-links from `docs/operations.md` and
`docs/troubleshooting.md`.

**Acceptance.**
- Every jargon term rendered in the detail panel appears in the glossary —
  asserted by the `<dt>` walk described above, not by a hand-maintained list.
- The page renders with the network disabled after one visit: an e2e test that
  aborts all requests after the first load still sees the shortcut table and the
  glossary, and the page issues no `/api/` request in the normal case either.
- The shortcut table and the `?` dialog are generated from one exported array;
  a test asserts the two render the same number of rows.
- `/help#retention` scrolls to and focuses the retention section, and the deep
  links from the expired-track copy land there.
- The primary nav is unchanged, on desktop and on the mobile bar.
- `@axe-core/playwright` is clean on `/help` in both themes, and the page is
  readable at the compact density setting.

**Not in scope.** Rendering `docs/*.md` at runtime, a search box, and any
tutorial or tour. This is a reference page.
